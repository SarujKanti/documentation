...
class DocumentationRepository {
    ...
    private fun excelFormulas() = BookSection(
        ...
        content = listOf(
            ...
            // ── Overview ─────────────────────────────────────────────────────
            ContentItem.ChapterHeader(
                "Excel Formulas & Functions",
                "Complete reference — 60+ essential formulas across every category"
            ),
            ContentItem.ScreenshotItem(
                "Formulas Tab Overview",
                "The Formulas tab is the control centre for all functions. Function Library group contains category buttons: AutoSum, Recently Used, Financial, Logical, Text, Date and Time, Lookup and Reference, Math and Trig, More Functions (Statistical, Engineering, Cube, Information, Compatibility, Web). Defined Names group manages named ranges. Formula Auditing group offers Trace Precedents/Dependents, Show Formulas (Ctrl+`), Error Checking, and Evaluate Formula for step-by-step debugging. Calculation group controls automatic vs manual recalculation.",
                "Click Formulas tab in the Excel ribbon"
            ),

            // ── Formula Basics ───────────────────────────────────────────────
            ContentItem.SectionHeading("Formula Basics"),
            ContentItem.BodyText("Every formula starts with = (equals sign). Excel evaluates it and shows the result in the cell."),
            ContentItem.NumberedStep(1, "Click a cell and type =", "The equals sign tells Excel this is a formula, not plain text."),
            ContentItem.NumberedStep(2, "Type the formula", "Use cell references (A1, B2:B10), operators (+, -, *, /, ^), and function names."),
            ContentItem.NumberedStep(3, "Press Enter", "Excel calculates and displays the result. The formula stays visible in the Formula Bar."),
            ContentItem.NumberedStep(4, "Operator precedence", "Excel follows BODMAS: Brackets → Exponents → Division/Multiplication → Addition/Subtraction. Use parentheses to override."),
            ContentItem.TipBox("Use cell references instead of hard-coded numbers. When the source data changes, all dependent formulas update instantly."),
            ContentItem.FormulaItem("=A1+B1", "Addition — adds values in A1 and B1"),
            ContentItem.FormulaItem("=A1-B1", "Subtraction — subtracts B1 from A1"),
            ContentItem.FormulaItem("=A1*B1", "Multiplication — multiplies A1 by B1"),
            ContentItem.FormulaItem("=A1/B1", "Division — divides A1 by B1 (shows #DIV/0! if B1 is zero)"),
            ContentItem.FormulaItem("=A1^2", "Exponentiation — raises A1 to the power of 2"),
            ContentItem.FormulaItem("=(A1+B1)*C1", "Brackets force addition before multiplication"),

            ContentItem.SectionHeading("Absolute vs Relative References"),
            ContentItem.BodyText("Cell references behave differently when you copy a formula. Understanding this prevents the most common formula mistakes."),
            ContentItem.FormulaItem("=A1*B1", "Relative — both addresses shift when you copy the formula to another cell"),
            ContentItem.FormulaItem("=A1*\$B\$1", "Mixed — B1 is locked (absolute), A1 adjusts. Use when multiplying a column by one fixed rate."),
            ContentItem.FormulaItem("=\$A\$1*\$B\$1", "Absolute — neither address changes when copied anywhere"),
            ContentItem.FormulaItem("=\$A1", "Column locked, row relative — useful for referencing one column across multiple rows"),
            ContentItem.FormulaItem("=A\$1", "Row locked, column relative — useful for referencing one row across multiple columns"),
            ContentItem.KeyShortcut("F4", "Cycle: A1 → \$A\$1 → A\$1 → \$A1 → A1 while editing a reference"),

            // ── Math & Statistical ───────────────────────────────────────────
            ContentItem.SectionHeading("Math & Statistical Functions"),
            ContentItem.ScreenshotItem(
                "AutoSum Function",
                "AutoSum (Alt+=) detects the contiguous range above or to the left of the active cell and inserts =SUM() automatically. The AutoSum dropdown arrow exposes five quick functions: Sum, Average, Count Numbers, Max, Min. Each inserts the corresponding function for the detected range. Selecting a range before pressing Alt+= fills all border totals at once — select a full table including a blank row/column for totals and press Alt+= to sum all columns and rows in one step.",
                "Formulas Tab → Function Library group → AutoSum button  OR  Alt+="
            ),
            ContentItem.SubHeading("SUM — Total a range"),
            ContentItem.FormulaItem("=SUM(A1:A10)", "Adds all numbers from A1 to A10"),
            ContentItem.FormulaItem("=SUM(A1:A10, C1:C10)", "Adds two non-contiguous ranges"),
            ContentItem.FormulaItem("=SUM(A:A)", "Adds the entire column A (ignores text and blanks)"),
            ContentItem.KeyShortcut("Alt+=", "AutoSum — inserts =SUM() for the detected range instantly"),
            ContentItem.SubHeading("AVERAGE, AVERAGEIF, AVERAGEIFS"),
            ContentItem.FormulaItem("=AVERAGE(B2:B20)", "Arithmetic mean of the range (ignores blank cells)"),
            ContentItem.FormulaItem("=AVERAGEIF(A:A, \"North\", B:B)", "Average of values in column B where column A equals 'North'"),
            ContentItem.FormulaItem("=AVERAGEIFS(C:C, A:A, \"North\", B:B, \">50\")", "Average of C where A='North' AND B>50"),
            ContentItem.SubHeading("MIN, MAX, MINIFS, MAXIFS"),
            ContentItem.FormulaItem("=MIN(C2:C50)", "Smallest number in the range"),
            ContentItem.FormulaItem("=MAX(C2:C50)", "Largest number in the range"),
            ContentItem.FormulaItem("=MINIFS(C:C, A:A, \"East\")", "Smallest value in C where A equals 'East' (Excel 2019+)"),
            ContentItem.FormulaItem("=MAXIFS(C:C, B:B, \">0\")", "Largest value in C where B is positive (Excel 2019+)"),
            ContentItem.SubHeading("COUNT, COUNTA, COUNTBLANK"),
            ContentItem.FormulaItem("=COUNT(D2:D100)", "Counts cells that contain numbers only"),
            ContentItem.FormulaItem("=COUNTA(D2:D100)", "Counts all non-empty cells (numbers, text, dates)"),
            ContentItem.FormulaItem("=COUNTBLANK(D2:D100)", "Counts empty / blank cells in the range"),
            ContentItem.SubHeading("ROUND, ROUNDUP, ROUNDDOWN"),
            ContentItem.FormulaItem("=ROUND(A1, 2)", "Rounds A1 to 2 decimal places (standard rounding)"),
            ContentItem.FormulaItem("=ROUNDUP(A1, 0)", "Always rounds UP to the nearest whole number"),
            ContentItem.FormulaItem("=ROUNDDOWN(A1, 0)", "Always rounds DOWN (truncates) to the nearest whole number"),
            ContentItem.FormulaItem("=MROUND(A1, 0.5)", "Rounds to the nearest multiple of 0.5"),
            ContentItem.SubHeading("Other Math Functions"),
            ContentItem.FormulaItem("=ABS(A1)", "Absolute value — removes the negative sign (e.g., ABS(-5) = 5)"),
            ContentItem.FormulaItem("=MOD(A1, 3)", "Remainder after dividing A1 by 3 (e.g., MOD(10,3) = 1)"),
            ContentItem.FormulaItem("=INT(A1)", "Rounds down to the nearest integer (INT(4.9)=4, INT(-4.1)=-5)"),
            ContentItem.FormulaItem("=TRUNC(A1, 2)", "Truncates to 2 decimal places without rounding"),
            ContentItem.FormulaItem("=SQRT(A1)", "Square root of A1"),
            ContentItem.FormulaItem("=POWER(A1, 3)", "Raises A1 to the power of 3 (same as A1^3)"),
            ContentItem.FormulaItem("=RAND()", "Random decimal between 0 and 1 (recalculates on every change)"),
            ContentItem.FormulaItem("=RANDBETWEEN(1, 100)", "Random whole number between 1 and 100"),
            ContentItem.FormulaItem("=CEILING(A1, 5)", "Rounds UP to the nearest multiple of 5"),
            ContentItem.FormulaItem("=FLOOR(A1, 5)", "Rounds DOWN to the nearest multiple of 5"),
            ContentItem.FormulaItem("=SUBTOTAL(9, B2:B100)", "SUM that ignores filtered-out rows (9=SUM, 1=AVERAGE, 2=COUNT, 4=MAX, 5=MIN)"),
            ContentItem.FormulaItem("=AGGREGATE(9,5,B2:B100)", "Like SUBTOTAL but also ignores errors and hidden rows"),
            ContentItem.FormulaItem("=PI()", "Returns the value of π (3.14159265358979…)"),
            ContentItem.FormulaItem("=SUMPRODUCT(A2:A10, B2:B10)", "Multiplies matching elements of two arrays then sums the products"),

            // ── Lookup & Reference ───────────────────────────────────────────
            ContentItem.SectionHeading("Lookup & Reference Functions"),
            ContentItem.ScreenshotItem(
                "VLOOKUP Example",
                "VLOOKUP (Vertical Lookup) syntax: =VLOOKUP(lookup_value, table_array, col_index_num, range_lookup). lookup_value = the value to find (cell reference or literal). table_array = the range to search — VLOOKUP always searches the LEFTMOST column. col_index_num = which column in the table to return (1 = the lookup column itself, 2 = next column, etc.). range_lookup = FALSE for exact match (most common), TRUE for approximate match (requires the lookup column sorted ascending — used for tax brackets, grade scales). Common errors: #N/A = value not found, #REF! = col_index_num exceeds table width, #VALUE! = col_index_num is less than 1.",
                "Type =VLOOKUP( in a cell  OR  Formulas Tab → Lookup and Reference → VLOOKUP"
            ),
            ContentItem.SubHeading("VLOOKUP — Vertical Lookup"),
            ContentItem.BodyText("VLOOKUP finds a value in the FIRST (leftmost) column of a range, then returns a value from a column you specify to its right."),
            ContentItem.FormulaItem("=VLOOKUP(A2, \$D:\$F, 2, FALSE)", "Finds A2 in column D, returns the value from column E. FALSE = exact match."),
            ContentItem.FormulaItem("=VLOOKUP(A2, Sheet2!\$A:\$C, 3, FALSE)", "Cross-sheet lookup — searches Sheet2 column A, returns column C value"),
            ContentItem.FormulaItem("=VLOOKUP(A2, \$D:\$F, 2, TRUE)", "Approximate match — finds the largest value ≤ A2. Table must be sorted ascending. Used for grade scales and tax brackets."),
            ContentItem.FormulaItem("=IFERROR(VLOOKUP(A2,\$D:\$F,2,FALSE),\"Not found\")", "Wraps VLOOKUP in IFERROR to show 'Not found' instead of #N/A"),
            ContentItem.TipBox("VLOOKUP limitation: it can only look right (returns columns to the right of the lookup column). For left-side lookups use INDEX+MATCH or XLOOKUP."),
            ContentItem.ScreenshotItem(
                "HLOOKUP Example",
                "HLOOKUP (Horizontal Lookup) is VLOOKUP rotated 90°. It searches the FIRST ROW (top row) of a range for a value, then returns a value from a row you specify below it. Syntax: =HLOOKUP(lookup_value, table_array, row_index_num, range_lookup). lookup_value = value to find in the top row. table_array = the horizontal range (headers in row 1, data below). row_index_num = which row to return (1 = top row, 2 = second row, etc.). range_lookup = FALSE for exact match, TRUE for approximate. HLOOKUP is best for horizontally-oriented lookup tables — e.g., months across the top, metrics below.",
                "Type =HLOOKUP( in a cell  OR  Formulas Tab → Lookup and Reference → HLOOKUP"
            ),
            ContentItem.SubHeading("HLOOKUP — Horizontal Lookup"),
            ContentItem.BodyText("HLOOKUP searches the FIRST ROW of a range for a value, then returns a value from a specified row below it. Use when your lookup table is arranged horizontally."),
            ContentItem.FormulaItem("=HLOOKUP(A2, \$A\$1:\$Z\$5, 3, FALSE)", "Finds A2 in row 1 of the range, returns the value in row 3 of the same column. FALSE = exact match."),
            ContentItem.FormulaItem("=HLOOKUP(\"Jan\", \$B\$1:\$M\$4, 2, FALSE)", "Finds 'Jan' in the header row, returns row 2 data (e.g., sales for January)"),
            ContentItem.FormulaItem("=HLOOKUP(A2, \$A\$1:\$Z\$3, 2, TRUE)", "Approximate match — finds largest header value ≤ A2 (table must be sorted left-to-right)"),
            ContentItem.NoteBox("VLOOKUP searches columns (vertical tables). HLOOKUP searches rows (horizontal tables). Both share the same limitation: they can only look in one direction. XLOOKUP and INDEX+MATCH overcome this."),
            ContentItem.SubHeading("XLOOKUP — Modern Universal Lookup (Microsoft 365 / Excel 2021+)"),
            ContentItem.BodyText("XLOOKUP replaces both VLOOKUP and HLOOKUP. It can look left, right, up, or down, returns a custom value if not found, and doesn't require sorting for approximate matches."),
            ContentItem.FormulaItem("=XLOOKUP(A2, B:B, C:C)", "Looks up A2 in column B, returns the matching value from column C"),
            ContentItem.FormulaItem("=XLOOKUP(A2, B:B, C:C, \"Not found\")", "Shows 'Not found' instead of #N/A when no match exists"),
            ContentItem.FormulaItem("=XLOOKUP(A2, B:B, C:C, \"—\", 0)", "4th arg = if_not_found, 5th arg = match_mode: 0=exact, -1=next smaller, 1=next larger, 2=wildcard"),
            ContentItem.FormulaItem("=XLOOKUP(A2, B:B, C:C, , , -1)", "Search from last row to first (reverse search — finds last occurrence)"),
            ContentItem.FormulaItem("=XLOOKUP(A2, B2:B10, C2:E10)", "Returns multiple columns at once (spills results into adjacent cells)"),
            ContentItem.ScreenshotItem(
                "INDEX MATCH Formula",
                "INDEX+MATCH is the classic power-user alternative to VLOOKUP that works in all Excel versions. INDEX returns the value at a specific row and column position in a range. MATCH finds the position (row number) of a value in a single row or column. Combined: MATCH finds WHERE the value is, then INDEX retrieves WHAT is at that position. This combination can look left (impossible in VLOOKUP), handle unsorted data, and is not affected by inserting or deleting columns (VLOOKUP breaks when columns shift). Syntax: =INDEX(return_range, MATCH(lookup_value, lookup_range, 0)). The 0 in MATCH means exact match.",
                "Type =INDEX(  then =MATCH(  OR  Formulas Tab → Lookup and Reference → INDEX / MATCH"
            ),
            ContentItem.SubHeading("INDEX — Return a value by position"),
            ContentItem.FormulaItem("=INDEX(C2:C100, 5)", "Returns the 5th value in the range C2:C100 (i.e., C6)"),
            ContentItem.FormulaItem("=INDEX(B2:D10, 3, 2)", "Returns the value at row 3, column 2 of the range B2:D10 (i.e., C4)"),
            ContentItem.SubHeading("MATCH — Find the position of a value"),
            ContentItem.FormulaItem("=MATCH(\"John\", A:A, 0)", "Returns the row number where 'John' appears in column A. 0 = exact match."),
            ContentItem.FormulaItem("=MATCH(A2, B2:B100, 1)", "Finds position of largest value ≤ A2 (requires ascending sort). Use for tiered lookups."),
            ContentItem.FormulaItem("=MATCH(A2, B2:B100, -1)", "Finds position of smallest value ≥ A2 (requires descending sort)"),
            ContentItem.SubHeading("INDEX + MATCH — The Power Combination"),
            ContentItem.BodyText("Combine INDEX and MATCH to create a flexible lookup that works in any direction. MATCH finds the row position, INDEX retrieves the value."),
            ContentItem.FormulaItem("=INDEX(C:C, MATCH(A2, B:B, 0))", "Finds A2 in column B, returns matching value from column C. Can look LEFT (unlike VLOOKUP)."),
            ContentItem.FormulaItem("=INDEX(A:A, MATCH(A2, C:C, 0))", "Left-side lookup — finds A2 in column C, returns value from column A (impossible with VLOOKUP)"),
            ContentItem.FormulaItem("=INDEX(\$C:\$E, MATCH(A2,\$B:\$B,0), MATCH(B2,\$C\$1:\$E\$1,0))", "Two-way lookup — finds row by A2, finds column by B2, returns the intersection value"),
            ContentItem.TipBox("INDEX+MATCH is safer than VLOOKUP when your table might have columns inserted or deleted — VLOOKUP's col_index_num breaks, but INDEX+MATCH uses named columns and stays correct."),
            ContentItem.SubHeading("CHOOSE — Pick from a list by number"),
            ContentItem.FormulaItem("=CHOOSE(2, \"Mon\", \"Tue\", \"Wed\", \"Thu\", \"Fri\")", "Returns the 2nd item = 'Tue'. First argument is the index (1–254)."),
            ContentItem.FormulaItem("=CHOOSE(WEEKDAY(TODAY()), \"Sun\", \"Mon\", \"Tue\", \"Wed\", \"Thu\", \"Fri\", \"Sat\")", "Returns today's day name based on WEEKDAY number"),
            ContentItem.SubHeading("OFFSET — Dynamic range reference"),
            ContentItem.FormulaItem("=OFFSET(A1, 2, 3)", "Returns the value 2 rows down and 3 columns right of A1 (i.e., D3)"),
            ContentItem.FormulaItem("=SUM(OFFSET(A1, 0, 0, 5, 1))", "Sums a range starting at A1 that is 5 rows tall and 1 column wide"),
            ContentItem.SubHeading("INDIRECT — Reference from a text string"),
            ContentItem.FormulaItem("=INDIRECT(\"A\"&B1)", "If B1 contains 5, returns the value in A5. Useful for dynamic references."),
            ContentItem.FormulaItem("=INDIRECT(C1&\"!A1\")", "If C1 contains a sheet name, returns cell A1 from that sheet"),
            ContentItem.SubHeading("ROW, COLUMN, ROWS, COLUMNS"),
            ContentItem.FormulaItem("=ROW(A5)", "Returns 5 (the row number of A5)"),
            ContentItem.FormulaItem("=COLUMN(C1)", "Returns 3 (the column number of C1)"),
            ContentItem.FormulaItem("=ROWS(A1:A10)", "Returns 10 (the number of rows in the range)"),
            ContentItem.FormulaItem("=COLUMNS(A1:E1)", "Returns 5 (the number of columns in the range)"),

            // ── Conditional / Criteria Functions ─────────────────────────────
            ContentItem.SectionHeading("Conditional & Criteria Functions"),
            ContentItem.SubHeading("COUNTIF & COUNTIFS"),
            ContentItem.FormulaItem("=COUNTIF(A:A, \"Apple\")", "Counts cells in column A that equal 'Apple'"),
            ContentItem.FormulaItem("=COUNTIF(A:A, \"App*\")", "Wildcard — counts cells starting with 'App' (Apple, Application, etc.)"),
            ContentItem.FormulaItem("=COUNTIF(B:B, \">100\")", "Counts cells in B with values greater than 100"),
            ContentItem.FormulaItem("=COUNTIF(B:B, \"<>0\")", "Counts cells in B that are not equal to zero"),
            ContentItem.FormulaItem("=COUNTIFS(A:A, \"North\", B:B, \">50\")", "Counts rows where A='North' AND B>50 — multiple criteria, all must match"),
            ContentItem.SubHeading("SUMIF & SUMIFS"),
            ContentItem.FormulaItem("=SUMIF(A:A, \"North\", B:B)", "Sums values in column B where the corresponding column A cell equals 'North'"),
            ContentItem.FormulaItem("=SUMIF(B:B, \">0\", C:C)", "Sums column C values where column B is positive"),
            ContentItem.FormulaItem("=SUMIFS(C:C, A:A, \"North\", B:B, \">50\")", "Sums column C where A='North' AND B>50 — note argument order: sum_range first, then criteria pairs"),
            ContentItem.TipBox("SUMIF = one condition. SUMIFS = multiple conditions. SUMIFS argument order: =SUMIFS(sum_range, criteria_range1, criteria1, criteria_range2, criteria2, ...)"),

            // ── Logical Functions ─────────────────────────────────────────────
            ContentItem.SectionHeading("Logical Functions"),
            ContentItem.SubHeading("IF — Single condition branch"),
            ContentItem.FormulaItem("=IF(A1>100, \"Pass\", \"Fail\")", "If A1 > 100, returns 'Pass'; otherwise returns 'Fail'"),
            ContentItem.FormulaItem("=IF(B2=\"\", \"Empty\", B2)", "If B2 is blank, shows 'Empty'; otherwise shows B2's value"),
            ContentItem.FormulaItem("=IF(A1>=90,\"A\",IF(A1>=80,\"B\",IF(A1>=70,\"C\",\"F\")))", "Nested IF — grades A/B/C/F. Stack up to 64 IFs, but IFS() is cleaner."),
            ContentItem.SubHeading("IFS — Multiple conditions without nesting"),
            ContentItem.FormulaItem("=IFS(A1>=90,\"A\",A1>=80,\"B\",A1>=70,\"C\",TRUE,\"F\")", "Tests conditions in order, returns the first match. TRUE as last condition acts as an 'else'. Available in Excel 2019+."),
            ContentItem.SubHeading("AND, OR, NOT"),
            ContentItem.FormulaItem("=AND(A1>0, B1>0)", "TRUE only if BOTH A1 and B1 are positive"),
            ContentItem.FormulaItem("=OR(A1>100, B1>100)", "TRUE if EITHER A1 or B1 exceeds 100"),
            ContentItem.FormulaItem("=NOT(A1>100)", "TRUE if A1 is NOT greater than 100"),
            ContentItem.FormulaItem("=IF(AND(A1>0, B1>0), \"Both positive\", \"Check values\")", "Combine AND inside IF for complex conditions"),
            ContentItem.SubHeading("IFERROR — Handle errors gracefully"),
            ContentItem.FormulaItem("=IFERROR(A1/B1, 0)", "If A1/B1 causes any error (e.g., #DIV/0!), returns 0 instead"),
            ContentItem.FormulaItem("=IFERROR(VLOOKUP(A2,B:C,2,FALSE), \"Not found\")", "Shows 'Not found' when VLOOKUP cannot locate the value"),
            ContentItem.SubHeading("IFNA — Handle #N/A errors specifically"),
            ContentItem.FormulaItem("=IFNA(VLOOKUP(A2,B:C,2,FALSE), \"Missing\")", "Only catches #N/A errors — leaves other errors (like #VALUE!) visible for debugging"),
            ContentItem.SubHeading("SWITCH — Match one value against multiple options"),
            ContentItem.FormulaItem("=SWITCH(A1,1,\"Jan\",2,\"Feb\",3,\"Mar\",\"Other\")", "If A1=1 returns 'Jan', if 2 returns 'Feb', if 3 returns 'Mar', else 'Other'. Available in Excel 2019+."),

            // ── Text Functions ────────────────────────────────────────────────
            ContentItem.SectionHeading("Text Functions"),
            ContentItem.SubHeading("Join text"),
            ContentItem.FormulaItem("=A1&\" \"&B1", "Ampersand operator — joins text: 'John' & ' ' & 'Smith' = 'John Smith'"),
            ContentItem.FormulaItem("=CONCAT(A1, \" \", B1)", "CONCAT joins multiple strings — replaces old CONCATENATE function"),
            ContentItem.FormulaItem("=TEXTJOIN(\", \", TRUE, A1:A10)", "Joins a range with a delimiter, skipping blanks (2nd arg TRUE = skip empties). E.g., 'Apple, Banana, Cherry'"),
            ContentItem.SubHeading("Extract text"),
            ContentItem.FormulaItem("=LEFT(A1, 3)", "First 3 characters from the left (e.g., LEFT('Smith', 3) = 'Smi')"),
            ContentItem.FormulaItem("=RIGHT(A1, 4)", "Last 4 characters from the right (e.g., RIGHT('Smith', 4) = 'mith')"),
            ContentItem.FormulaItem("=MID(A1, 3, 5)", "5 characters starting from position 3 (e.g., MID('ExcelFormulas', 3, 5) = 'celFo')"),
            ContentItem.SubHeading("Find & measure"),
            ContentItem.FormulaItem("=LEN(A1)", "Number of characters in A1 (including spaces)"),
            ContentItem.FormulaItem("=FIND(\"@\", A1)", "Position of '@' in A1 — case-sensitive, returns #VALUE! if not found"),
            ContentItem.FormulaItem("=SEARCH(\"excel\", A1)", "Position of 'excel' — NOT case-sensitive, supports wildcards (* and ?)"),
            ContentItem.FormulaItem("=FIND(\"@\", A1)-1", "Characters before '@' — combine with LEFT to extract email username"),
            ContentItem.SubHeading("Change & clean text"),
            ContentItem.FormulaItem("=UPPER(A1)", "Converts all text to UPPERCASE"),
            ContentItem.FormulaItem("=LOWER(A1)", "Converts all text to lowercase"),
            ContentItem.FormulaItem("=PROPER(A1)", "Capitalises the First Letter Of Every Word"),
            ContentItem.FormulaItem("=TRIM(A1)", "Removes leading, trailing, and double spaces from text"),
            ContentItem.FormulaItem("=SUBSTITUTE(A1, \"old\", \"new\")", "Replaces every occurrence of 'old' with 'new' in A1"),
            ContentItem.FormulaItem("=SUBSTITUTE(A1, \" \", \"\")", "Removes all spaces from text (e.g., phone number cleanup)"),
            ContentItem.FormulaItem("=REPLACE(A1, 3, 2, \"XX\")", "Replaces 2 characters starting at position 3 with 'XX'"),
            ContentItem.SubHeading("Convert text"),
            ContentItem.FormulaItem("=TEXT(A1, \"dd/mm/yyyy\")", "Converts a date value to text with a custom format (e.g., 01/01/2025)"),
            ContentItem.FormulaItem("=TEXT(A1, \"#,##0.00\")", "Formats a number as text with commas and 2 decimal places"),
            ContentItem.FormulaItem("=VALUE(A1)", "Converts text that looks like a number into an actual number Excel can calculate"),

            // ── Date & Time ───────────────────────────────────────────────────
            ContentItem.SectionHeading("Date & Time Functions"),
            ContentItem.ScreenshotItem(
                "Date Functions",
                "Excel stores dates as serial numbers (1 = 1 January 1900). TODAY() returns today's serial number; displayed as a date because of cell formatting. Arithmetic on dates works because of this: TODAY()-A1 gives the number of days since the date in A1. Date functions extract parts (YEAR, MONTH, DAY) or build dates (DATE). DATEDIF is a hidden legacy function not shown in AutoComplete but still works — it calculates the difference in days (D), months (M), or years (Y) between two dates. NETWORKDAYS excludes weekends and optional holidays. WORKDAY returns a future date after N working days.",
                "Formulas Tab → Function Library group → Date and Time"
            ),
            ContentItem.FormulaItem("=TODAY()", "Today's date (updates every time the workbook recalculates)"),
            ContentItem.FormulaItem("=NOW()", "Current date AND time (dynamic — updates on recalculation)"),
            ContentItem.FormulaItem("=DATE(2025, 12, 31)", "Constructs the date 31 December 2025 from year, month, day numbers"),
            ContentItem.FormulaItem("=YEAR(A1)", "Extracts the 4-digit year from a date in A1"),
            ContentItem.FormulaItem("=MONTH(A1)", "Extracts the month number (1–12) from a date"),
            ContentItem.FormulaItem("=DAY(A1)", "Extracts the day number (1–31) from a date"),
            ContentItem.FormulaItem("=HOUR(A1)", "Extracts the hour (0–23) from a time value"),
            ContentItem.FormulaItem("=MINUTE(A1)", "Extracts the minute (0–59) from a time value"),
            ContentItem.FormulaItem("=SECOND(A1)", "Extracts the second (0–59) from a time value"),
            ContentItem.FormulaItem("=TODAY()-A1", "Number of days since the date in A1 (age in days, days overdue, etc.)"),
            ContentItem.FormulaItem("=DATEDIF(A1, TODAY(), \"Y\")", "Complete years between A1 and today — use for age in years. 'M' = months, 'D' = days"),
            ContentItem.FormulaItem("=DAYS(B1, A1)", "Number of days between two dates (B1 − A1). Simpler than DATEDIF for day counts."),
            ContentItem.FormulaItem("=EDATE(A1, 3)", "Date exactly 3 months after A1 (handles month-end correctly)"),
            ContentItem.FormulaItem("=EOMONTH(A1, 0)", "Last day of the same month as A1. EOMONTH(A1,1) = last day of next month."),
            ContentItem.FormulaItem("=WEEKDAY(A1, 2)", "Day of week number: 2=Monday(1)…Sunday(7). Arg 2 = starts week on Monday."),
            ContentItem.FormulaItem("=WEEKNUM(A1)", "Week number of the year (1–53)"),
            ContentItem.FormulaItem("=NETWORKDAYS(A1, B1)", "Working days between A1 and B1 (excludes Saturdays and Sundays)"),
            ContentItem.FormulaItem("=NETWORKDAYS(A1, B1, Holidays!A:A)", "Working days excluding a named holiday list in another range"),
            ContentItem.FormulaItem("=WORKDAY(A1, 10)", "Date that is 10 working days after A1 (skips weekends)"),
            ContentItem.TipBox("Format date formula results as dates: select the cell → Ctrl+1 → Number tab → Date → choose a format. Excel stores dates as numbers; formatting makes them readable."),

            // ── Financial Functions ───────────────────────────────────────────
            ContentItem.SectionHeading("Financial Functions"),
            ContentItem.BodyText("Financial functions help calculate loan payments, investment values, interest rates, and returns. All assume cash paid out is negative, cash received is positive."),
            ContentItem.SubHeading("PMT — Loan / mortgage payment"),
            ContentItem.FormulaItem("=PMT(5%/12, 60, -10000)", "Monthly payment for a £10,000 loan at 5% annual interest over 60 months (5 years)"),
            ContentItem.FormulaItem("=PMT(B1/12, B2*12, -B3)", "Dynamic PMT: B1=annual rate, B2=years, B3=loan amount"),
            ContentItem.SubHeading("PV — Present value"),
            ContentItem.FormulaItem("=PV(5%/12, 60, -200)", "Present value of 60 monthly payments of £200 at 5% annual interest"),
            ContentItem.SubHeading("FV — Future value"),
            ContentItem.FormulaItem("=FV(6%/12, 120, -500)", "Future value after 120 monthly payments of £500 at 6% annual interest (savings growth)"),
            ContentItem.SubHeading("RATE — Calculate the interest rate"),
            ContentItem.FormulaItem("=RATE(60, -200, 10000)*12", "Annual interest rate for a £10,000 loan with £200/month over 60 months"),
            ContentItem.SubHeading("NPER — Number of periods"),
            ContentItem.FormulaItem("=NPER(5%/12, -200, 10000)", "How many months to repay a £10,000 loan with £200/month at 5% annual"),
            ContentItem.SubHeading("NPV — Net present value"),
            ContentItem.FormulaItem("=NPV(10%, B2:B6)", "Present value of future cash flows in B2:B6 discounted at 10% — add initial investment separately"),
            ContentItem.FormulaItem("=-A1+NPV(10%, B2:B6)", "Complete NPV: subtract initial outlay (A1) from the discounted cash flows"),
            ContentItem.SubHeading("IRR — Internal rate of return"),
            ContentItem.FormulaItem("=IRR(A1:A6)", "Rate of return that makes NPV=0 for cash flows in A1:A6 (A1 should be negative initial investment)"),
            ContentItem.SubHeading("Depreciation"),
            ContentItem.FormulaItem("=SLN(10000, 1000, 5)", "Straight-line depreciation: asset cost £10,000, salvage £1,000, life 5 years → £1,800/year"),
            ContentItem.FormulaItem("=DB(10000, 1000, 5, 1)", "Declining balance depreciation for period 1"),

            // ── Dynamic Array Functions (Microsoft 365 / Excel 2021+) ─────────
            ContentItem.SectionHeading("Dynamic Array Functions  (Microsoft 365 / Excel 2021+)"),
            ContentItem.BodyText("Dynamic array formulas automatically spill results into multiple cells. Enter in ONE cell — Excel fills the adjacent cells automatically. A blue border marks the spill range."),
            ContentItem.ScreenshotItem(
                "Dynamic Array Spill",
                "Dynamic arrays were introduced in Excel 365 and Excel 2021. A single formula entered in one cell automatically spills its results into as many adjacent cells as needed — no Ctrl+Shift+Enter required. A blue border surrounds the spill range. If something blocks the spill range, Excel shows a #SPILL! error. Click the formula cell and the blue dotted border shows the intended spill range. The hash (#) reference operator refers to the whole spill range: if A1 contains a FILTER formula, A1# refers to all its results. SORT, UNIQUE, SEQUENCE, FILTER, RANDARRAY, and XLOOKUP all produce spill arrays.",
                "Enter formula in one cell → results automatically fill into adjacent cells (Excel 365 / 2021+ only)"
            ),
            ContentItem.SubHeading("FILTER — Extract rows matching criteria"),
            ContentItem.FormulaItem("=FILTER(A2:C100, B2:B100=\"North\")", "Returns all rows where column B equals 'North' — results spill automatically"),
            ContentItem.FormulaItem("=FILTER(A2:C100, (B2:B100=\"North\")*(C2:C100>50))", "Multiple criteria with AND — multiply the conditions"),
            ContentItem.FormulaItem("=FILTER(A2:C100, (B2:B100=\"North\")+(C2:C100>100), \"No data\")", "OR criteria — add the conditions. 3rd argument shows if no results match."),
            ContentItem.SubHeading("SORT & SORTBY — Sort a range"),
            ContentItem.FormulaItem("=SORT(A2:C20)", "Returns the range A2:C20 sorted by the first column ascending"),
            ContentItem.FormulaItem("=SORT(A2:C20, 3, -1)", "Sorts by the 3rd column descending (-1). Use 1 for ascending."),
            ContentItem.FormulaItem("=SORTBY(A2:B20, B2:B20, -1)", "Sorts A2:B20 by column B descending — sort key can be outside the return range"),
            ContentItem.SubHeading("UNIQUE — Extract distinct values"),
            ContentItem.FormulaItem("=UNIQUE(A2:A100)", "Returns a list of unique (deduplicated) values from A2:A100"),
            ContentItem.FormulaItem("=UNIQUE(A2:A100, FALSE, TRUE)", "Returns values that appear exactly once (not just distinct — truly unique only)"),
            ContentItem.SubHeading("SEQUENCE — Generate a number series"),
            ContentItem.FormulaItem("=SEQUENCE(10)", "Generates 1, 2, 3, … 10 in a column"),
            ContentItem.FormulaItem("=SEQUENCE(5, 3, 10, 5)", "5 rows × 3 columns, starting at 10, incrementing by 5"),
            ContentItem.SubHeading("XMATCH — Position of a value (modern MATCH)"),
            ContentItem.FormulaItem("=XMATCH(A2, B:B)", "Returns the position of A2 in column B — exact match, no third argument needed"),
            ContentItem.FormulaItem("=XMATCH(A2, B:B, 0, -1)", "Finds last occurrence (search from bottom). 3rd arg: 0=exact, 1=next larger, -1=next smaller, 2=wildcard"),

            // ── Information Functions ─────────────────────────────────────────
            ContentItem.SectionHeading("Information & Error-Checking Functions"),
            ContentItem.SubHeading("IS functions — Check the type of a cell"),
            ContentItem.FormulaItem("=ISNUMBER(A1)", "TRUE if A1 contains a number; FALSE for text, blanks, or errors"),
            ContentItem.FormulaItem("=ISTEXT(A1)", "TRUE if A1 contains text"),
            ContentItem.FormulaItem("=ISBLANK(A1)", "TRUE if A1 is completely empty (FALSE if it contains a space or formula returning empty string)"),
            ContentItem.FormulaItem("=ISERROR(A1)", "TRUE if A1 contains any error (#DIV/0!, #N/A, #VALUE!, #REF!, #NAME?, #NUM!, #NULL!)"),
            ContentItem.FormulaItem("=ISNA(A1)", "TRUE only for #N/A errors (lookup 'not found' — more precise than ISERROR)"),
            ContentItem.FormulaItem("=ISFORMULA(A1)", "TRUE if A1 contains a formula (not a hard-coded value)"),
            ContentItem.SubHeading("TYPE — Return a numeric code for data type"),
            ContentItem.FormulaItem("=TYPE(A1)", "Returns: 1=number, 2=text, 4=logical (TRUE/FALSE), 8=formula, 16=error, 64=array"),
            ContentItem.SubHeading("CELL — Information about a cell"),
            ContentItem.FormulaItem("=CELL(\"address\", A1)", "Returns the absolute address of A1 as text: \"\$A\$1\""),
            ContentItem.FormulaItem("=CELL(\"type\", A1)", "Returns 'b' (blank), 'l' (label/text), or 'v' (value/number)"),

            // ── Named Ranges & Defined Names ──────────────────────────────────
            ContentItem.SectionHeading("Named Ranges — Make Formulas Readable"),
            ContentItem.BodyText("Instead of writing =SUM(B2:B100), define the range as 'Sales' and write =SUM(Sales). Named ranges make formulas self-documenting and easier to audit."),
            ContentItem.NumberedStep(1, "Select the range", "Highlight the cells you want to name."),
            ContentItem.NumberedStep(2, "Click the Name Box", "The Name Box is left of the formula bar — it shows the active cell address (e.g., B2). Click it, type a name, press Enter."),
            ContentItem.NumberedStep(3, "Use the name in formulas", "Type =SUM(Sales) — Excel substitutes the named range automatically."),
            ContentItem.NumberedStep(4, "Manage names", "Formulas tab → Defined Names group → Name Manager — view, edit, delete, or add names."),
            ContentItem.KeyShortcut("Ctrl+F3", "Open Name Manager directly"),
            ContentItem.TipBox("Name entire columns (e.g., name column B 'Revenue') and then use =SUM(Revenue) anywhere in the workbook. Names work across sheets."),

            // ── Absolute vs Relative ─────────────────────────────────────────
            ContentItem.SectionHeading("Absolute vs Relative References"),
            ContentItem.BodyText("When you copy a formula, relative references adjust automatically. Absolute references (with \$) stay fixed. Understanding this is essential for building reusable formula templates."),
            ContentItem.FormulaItem("=A1*B1", "Relative — A1 and B1 both shift when copied"),
            ContentItem.FormulaItem("=A1*\$B\$1", "B1 is absolute — locked to column B row 1 when copied anywhere"),
            ContentItem.FormulaItem("=\$A\$1*\$B\$1", "Both absolute — neither changes regardless of where you paste"),
            ContentItem.FormulaItem("=\$A1", "Column locked (A stays), row relative (adjusts) — useful for column-specific references"),
            ContentItem.FormulaItem("=A\$1", "Row locked (1 stays), column relative (adjusts) — useful for row-specific references"),
            ContentItem.KeyShortcut("F4", "While editing a cell reference: cycles A1 → \$A\$1 → A\$1 → \$A1 → A1"),
            ContentItem.KeyShortcut("Ctrl+`", "Toggle Show Formulas — displays all formula text instead of results"),
            ContentItem.KeyShortcut("Ctrl+[", "Select Precedents — jumps to cells that feed into the selected formula"),
        )
    )
